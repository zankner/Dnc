...
import numpy as np
import allocation_weighting, sorting_indices, usage_vector, write_weight_interpolation, memory_retention_vector
...
memory_retention_vectorfortest = tf.convert_to_tensor(np.array([[1, 2.2, 4.3, 4, 1.2], [4, 2.4, 2.3, 5, 1.2]])) #1xN
...
test_usage_vector = usage_vector.usage_vector(memory_retention_vectorfortest, prev_usage_vector, prev_write_vector)
...
forget_gate_list = [tf.convert_to_tensor(np.array([0.2, 0.4])), tf.convert_to_tensor(np.array([0.5, 0.1]))]
read_weighting_prev = tf.convert_to_tensor(np.transpose(np.array([[1, 2.2, 120, 4, 1.2], [3, 5, 7, 5.43, 1.22]])))
memory_retention_test = memory_retention_vector.memory_retention_vector(forget_gate_list, read_weighting_prev)
print(memory_retention_test)
